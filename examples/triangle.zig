...
pub fn main() !void {
    ...
    const vow_stream = try vow.spawn(.{
        // useful to debug the vowexe
        // .write_exe_path = "/tmp/vowexe",
    });

    var vow_write_buf: [1000]u8 = undefined;
    var vow_stream_writer = vow_stream.writer(&vow_write_buf);
    const vow_writer = &vow_stream_writer.interface;
    ...
    }) catch return vow_stream_writer.err.?;
    vow.writeShader(vow_writer, .vert, shader_triangle_vert) catch return vow_stream_writer.err.?;
    vow.writeShader(vow_writer, .frag, shader_triangle_frag) catch return vow_stream_writer.err.?;
    ...
    vow.writeClearColor(vow_writer, .{ 0, 0, 0, 1 }) catch return vow_stream_writer.err.?;
    vow.writePlaceholder(vow_writer, .{ .map_on_present = 1 }) catch return vow_stream_writer.err.?;
    vow_writer.flush() catch return vow_stream_writer.err.?;
    ...
    while (true) {
        ...
        if (frame_now >= next_frame_time) {
            ...
            vow.writeUpdate(vow_writer, std.mem.asBytes(&angle)) catch return vow_stream_writer.err.?;
            vow_writer.flush() catch return vow_stream_writer.err.?;
        }
    }
}

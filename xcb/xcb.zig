...
pub const Colormap = enum(u32) {
    ...
    pub fn format(cm: Colormap, writer: *std.Io.Writer) error{WriteFailed}!void {
        try fmtEnum(cm).format(writer);
    }
};
...
pub const Cursor = enum(u32) {
    ...
    pub fn format(cursor: Cursor, writer: *std.Io.Writer) error{WriteFailed}!void {
        try fmtEnum(cursor).format(writer);
    }
};
...
pub fn connect(display: ?[:0]const u8) ConnectError!struct { *Connection, u8 } {
    ...
    const connection = c.xcb_connect(if (display) |d| d.ptr else null, &screen_count);
    const err = c.xcb_connection_has_error(connection);
    return switch (err) {
...
pub const connection_has_error = c.xcb_connection_has_error;
pub const flush = c.xcb_flush;
pub const get_setup = c.xcb_get_setup;
pub const setup_roots_iterator = c.xcb_setup_roots_iterator;
pub const screen_next = c.xcb_screen_next;
pub const generate_id = c.xcb_generate_id;
pub const change_property = c.xcb_change_property;
pub const poll_for_event = c.xcb_poll_for_event;
pub const intern_atom = c.xcb_intern_atom;
pub const intern_atom_reply = c.xcb_intern_atom_reply;
pub const get_file_descriptor = c.xcb_get_file_descriptor;
pub const map_window = c.xcb_map_window;

pub const c = struct {
    pub extern "xcb" fn xcb_connect(display: ?[*:0]const u8, screenp: *c_int) callconv(.c) *Connection;
    pub extern "xcb" fn xcb_disconnect(*Connection) callconv(.c) void;
    pub extern "xcb" fn xcb_connection_has_error(*Connection) callconv(.c) c_int;
    pub extern "xcb" fn xcb_flush(*Connection) callconv(.c) c_int;
    pub extern "xcb" fn xcb_get_setup(*Connection) callconv(.c) *const Setup;
    pub extern "xcb" fn xcb_setup_roots_iterator(*const Setup) callconv(.c) ScreenIterator;
    pub extern "xcb" fn xcb_screen_next(*ScreenIterator) callconv(.c) void;
    pub extern "xcb" fn xcb_generate_id(*Connection) callconv(.c) Resource;
    pub extern "xcb" fn xcb_create_window(
        *Connection,
        depth: u8,
        wid: Window,
        parent: Window,
    ...
    pub extern "xcb" fn xcb_change_property(
        *Connection,
        mode: PropMode,
        window: Window,
        property: Atom,
    ...
    pub extern "xcb" fn xcb_poll_for_event(*Connection) callconv(.c) ?*GenericEvent;
    pub extern "xcb" fn xcb_intern_atom(
        *Connection,
        only_if_exists: u8,
        name_len: u16,
        name: [*:0]const u8,
    ) InternAtomCookie;
    pub extern "xcb" fn xcb_intern_atom_reply(
        *Connection,
        InternAtomCookie,
        ?**GenericError,
    ) callconv(.c) ?*InternAtomReply;
    pub extern "xcb" fn xcb_get_file_descriptor(c: *Connection) callconv(.c) c_int;
    pub extern "xcb" fn xcb_map_window(*Connection, Window) callconv(.c) VoidCookie;
};

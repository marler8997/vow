...
pub const RasterDesc = vowproto.RasterDesc;
pub const RenderTargetTransport = vowproto.RenderTargetTransport;
pub const RenderTargetInfo = vowproto.RenderTargetInfo;
...
pub fn writeRasterDesc(writer: *std.Io.Writer, desc: vowproto.RasterDesc) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.raster_desc));
    try writer.writeAll(std.mem.asBytes(&desc));
}
pub fn writeBegin(writer: *std.Io.Writer, named: struct {
    map_on_present: u8,
}) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.begin));
    try writer.writeByte(named.map_on_present);
}
pub fn writeBeginRenderPass(writer: *std.Io.Writer, clear_color: [4]f32) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.begin_render_pass));
    try writer.writeAll(std.mem.asBytes(&clear_color));
}
pub fn writeEndRenderPass(writer: *std.Io.Writer) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.end_render_pass));
}
pub fn writePresent(writer: *std.Io.Writer) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.present));
}
pub fn writeDraw(writer: *std.Io.Writer, named: struct {
    vertex_count: u32,
    memfd_id: u32,
    vertex_offset: u64,
    pixel_offset: u64,
}) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.draw));
    try writer.writeAll(std.mem.asBytes(&named.vertex_count));
    try writer.writeAll(std.mem.asBytes(&named.memfd_id));
    try writer.writeAll(std.mem.asBytes(&named.vertex_offset));
    try writer.writeAll(std.mem.asBytes(&named.pixel_offset));
}

pub fn writeGpuMalloc(writer: *std.Io.Writer, memfd_id: u32) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.gpu_malloc));
    try writer.writeAll(std.mem.asBytes(&memfd_id));
}

pub fn writeGpuUpload(writer: *std.Io.Writer, memfd_id: u32) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.gpu_upload));
    try writer.writeAll(std.mem.asBytes(&memfd_id));
}

pub fn writeBindPipeline(writer: *std.Io.Writer, pipeline_id: u8) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.bind_pipeline));
    try writer.writeByte(pipeline_id);
}

pub fn writeGpuFree(writer: *std.Io.Writer, memfd_id: u32) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.gpu_free));
    try writer.writeAll(std.mem.asBytes(&memfd_id));
}

pub fn writeCreateDevice(writer: *std.Io.Writer, validation_layers: ValidationLayers) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.create_device));
    try writer.writeByte(@intFromEnum(validation_layers));
}

pub fn writeCreateRenderTargets(writer: *std.Io.Writer, named: struct {
    width: u32,
    height: u32,
    count: u8,
}) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.create_render_targets));
    try writer.writeAll(std.mem.asBytes(&named.width));
    try writer.writeAll(std.mem.asBytes(&named.height));
    try writer.writeByte(named.count);
}

pub fn writeSubmit(writer: *std.Io.Writer, buffer_index: u8) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.submit));
    try writer.writeByte(buffer_index);
}

pub fn writeBeginRenderPassDmabuf(writer: *std.Io.Writer, buffer_index: u8, clear_color: [4]f32) error{WriteFailed}!void {
    try writer.writeByte(@intFromEnum(Request.begin_render_pass));
    try writer.writeByte(buffer_index);
    try writer.writeAll(std.mem.asBytes(&clear_color));
}

/// Read a render target response from the vow socket.
/// Returns the dmabuf fd and metadata.
/// Control message struct for receiving one fd via SCM_RIGHTS.
...
pub fn readRenderTargetResponse(socket_fd: std.posix.fd_t) !struct { fd: std.posix.fd_t, info: RenderTargetInfo } {
    var info: RenderTargetInfo = undefined;
    var iov = [_]std.posix.iovec{
        .{ .base = @ptrCast(std.mem.asBytes(&info)), .len = @sizeOf(RenderTargetInfo) },
    };
    var cmsg_storage: CmsgFd = undefined;
    var msg: std.posix.msghdr = .{
        .name = null,
        .namelen = 0,
        .iov = &iov,
        .iovlen = iov.len,
        .control = @ptrCast(&cmsg_storage),
        .controllen = @sizeOf(CmsgFd),
        .flags = 0,
    };

    var total_received: usize = 0;
    while (total_received < @sizeOf(RenderTargetInfo)) {
        const rc = std.os.linux.recvmsg(socket_fd, &msg, 0);
        const received = switch (std.posix.errno(rc)) {
            .SUCCESS => rc,
            .INTR => continue,
            .AGAIN => return error.WouldBlock,
            .CONNRESET => return error.ConnectionResetByPeer,
            else => |errno| {
                std.log.err("recvmsg failed: {}", .{errno});
                return error.Unexpected;
            },
        };
    ...
    if (msg.controllen < @sizeOf(CmsgFd)) return error.NoFdReceived;
    if (cmsg_storage.level != std.posix.SOL.SOCKET or cmsg_storage.@"type" != 1) return error.NoFdReceived;

    return .{ .fd = cmsg_storage.data, .info = info };
}
